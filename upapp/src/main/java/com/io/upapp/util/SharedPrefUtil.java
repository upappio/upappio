...
public class SharedPrefUtil {
    ...
    public SharedPrefUtil(Context appContext) {
        ...
        mySharedPref = shareContext.getSharedPreferences("upApp", Context.MODE_PRIVATE);
    }
    public void clear() {
        try {
    ...
    public boolean getBolValue(String key,boolean defaultValue) {
        boolean value = false;
        try {
            value = mySharedPref.getBoolean(key, defaultValue);
        } catch (Exception e) {
            System.out.println("getSharedPref ERROR: " + e.getMessage());
        }
        return value;
